...
if (isset($datos['accion']) && $datos['accion'] === 'editar' && isset($datos['idUsuario'])) {
    $control = new abmUsuarioLogin();
    $objUsuarioRol = new abmUsuarioRol();
    $resp = convert_array($control->buscar(['idusuario' => $datos['idUsuario']]));
    if ($resp && isset($resp[0])) {
        $persona = $resp[0];
        // Obtenemos los roles actuales del usuario
        $roles = convert_array($objUsuarioRol->buscar(['idusuario' => $persona['idUsuario']]));
                ...
                <form action="accion.php" method="post" id="formUsuario" name="formUsuario" novalidate>
                    ...
                    <?php if (!$esNuevaPersona) { ?>
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control bg-light" id="idusuario" name="idusuario" value="<?php echo htmlspecialchars($persona["idUsuario"]); ?>" readonly>
                        </div>
                    <?php } ?>

                    <div class="mb-3">
                        <label for="usnombre" class="form-label text-secondary"><strong>Nombre:</strong></label>
                        <input type="text" class="form-control" id="usnombre" name="usnombre" value="<?php echo htmlspecialchars($esNuevaPersona ? '' : $persona["usNombre"]); ?>" required>
                        <div class="invalid-feedback">Ingrese un nombre válido.</div>
                    </div>

                    <div class="mb-3">
                        <label for="uspass" class="form-label text-secondary"><strong>Contraseña:</strong></label>
                        <input type="password" class="form-control" id="uspass" name="uspass" value="<?php echo htmlspecialchars($esNuevaPersona ? '' : $persona["usPass"]); ?>" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" class="form-control" id="usmail" name="usmail" value="<?php echo htmlspecialchars($esNuevaPersona ? '' : $persona["usMail"]); ?>" required>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check">
                            ...
                            <input class="form-check-input" type="checkbox" id="rol_<?php echo $rol['idRol']; ?>" name="usrol[]" value="<?php echo $rol['idRol']; ?>" <?php if (isset($roles) && in_array($rol['idRol'], array_column($roles, 'idRol'))) echo 'checked'; ?>>
